...
const getFullFileStats = ({ editorInfo, }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (exports.getMacroInfoOfFile(editorInfo, txt).entireFileRightMost > 3000) {
        return;
    }
    ...
    if (extension_1.glo.colorDecoratorsInStyles) {
        const dataArr = yield vscode.commands.executeCommand("vscode.executeDocumentColorProvider", editorInfo.editorRef.document.uri);
        exports.generateTextLinesMap(txt, editorInfo);
        // const linesArr = txt.split(`\n`);
        // console.log(linesArr);
        // console.log(dataArr);
        if (dataArr && dataArr.length >= 1) {
            const startingPositions = dataArr.map((item) => {
                return {
    ...
    if ((extension_1.glo.analyzeCurlyBrackets ||
        ...
        */
        // return;
        let foundbrackets = extension_1.bracketManager.updateDocument(document, editorInfo);
        if (foundbrackets) {
