...
class RelativePath {
    ...
    initializeWatcher() {
        ...
        this._watcher = vscode_1.workspace.createFileSystemWatcher("**/*.*", IGNORE_CREATE_EVENTS, IGNORE_CHANGE_EVENTS, IGNORE_DELETE_EVENTS);
    ...
    updateFiles(skipOpen = false) {
        ...
        if (this._pausedSearch) {
            ...
            this._myGlob = new glob_1.Glob(this._workspacePath + this._configuration.get("includeGlob"), {
    ...
    searchWorkspace(skipOpen = false) {
        ...
        let info = vscode_1.window.showQuickPick([emptyItem], {
    ...
    showQuickPick(items, editor) {
        if (items) {
            ...
            let pickResult;
            pickResult = vscode_1.window.showQuickPick(paths, {
    ...
    returnRelativeLink(item, editor) {
        if (item) {
            ...
            if (this._configuration.removeLeadingDot &&
    ...
    findRelativePath() {
        ...
        const allowQuickFilter = this._configuration.searchCountLimit > this._fileNames.length;
        if (allowQuickFilter) {
            ...
            input.then((val) => {
                ...
                this.showQuickPick(this._fileNames.filter((item) => item.toLowerCase().indexOf(val.toLowerCase()) >
                    -1), editor);
            }, () => {
                return;
            });
